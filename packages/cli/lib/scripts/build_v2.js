...
async function build(isDev = false) {
    ...
    const defaultConfig = {
        input: {
            ...
            plugins: [nodeResolve(), commonjs(), filesize(), babel(babelConfig), ...tsPlugins],
            external: id => {
    ...
    const umdConfig = {
        input: {
            ...
            plugins: [
                ...
                babel({
                    ...babelConfig,
                    exclude: ["node_modules/**", "../../node_modules/core-js/**", /.*node_modules\/.*/],
                }),
                ...tsPlugins,
            ],
            external: id => {
    ...
    configList.map(async config => {
        const { input, output, watch, babel: customBabelConfig = null, ...otherProps } = config || {};
            ...
            watcher.on("event", event => {
                switch (event.code) {
                    case "END":
                        ...
                        runSh("yalc publish " + packageDir + " --update", null, { cwd: cliDir });
                        break;
                    case "ERROR":
